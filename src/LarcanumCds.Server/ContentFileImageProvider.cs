...
{
	public ContentFileImageProvider(
	...
	{
		return new VirtualFileProvider(sourceSettings.ResolveDataPath(environment),
			sourceSettings.ImageProcessorPrefix);
	}
		...
		public IFileInfo GetFileInfo(string subpath)
		{
			...
			if (!fullPath.StartsWith(_physicalRootPath))
			{
				throw new InvalidOperationException(
					$"Path {fullPath} is navigating outside of the physical root path {_physicalRootPath} of this provider");
			}
